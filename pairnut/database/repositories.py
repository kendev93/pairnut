...
def validate_variety_input(
    name: str, code_prefix: str, tolerance_mm: float
) -> tuple[str, str, float]:
    normalized_name = name.strip() if isinstance(name, str) else ""
    normalized_prefix = (
        code_prefix.strip().upper() if isinstance(code_prefix, str) else ""
    )
...
def create_variety(name: str, code_prefix: str, tolerance_mm: float = 1.0) -> int:
    name, code_prefix, tolerance_mm = validate_variety_input(
        name, code_prefix, tolerance_mm
    )
    timestamp = now_str()
...
def update_variety(
    variety_id: int, name: str, code_prefix: str, tolerance_mm: float
) -> None:
    name, code_prefix, tolerance_mm = validate_variety_input(
        name, code_prefix, tolerance_mm
    )
...
def get_variety(variety_id: int) -> dict[str, Any] | None:
    with db_connection() as conn:
        row = conn.execute(
            "SELECT * FROM varieties WHERE id = ?", (variety_id,)
        ).fetchone()
...
def list_varieties() -> list[dict[str, Any]]:
    with db_connection() as conn:
        rows = conn.execute(
            "SELECT * FROM varieties ORDER BY name COLLATE NOCASE"
        ).fetchall()
...
def get_walnut_by_serial_and_variety(
    serial_no: str, variety_id: int
) -> dict[str, Any] | None:
    with db_connection() as conn:
        row = conn.execute(
...
def list_walnuts(
    variety_id: int | None = None, include_locked: bool = True
) -> list[dict[str, Any]]:
...
def upsert_walnut_image(
    walnut_id: int, face_no: int, original_filename: str, stored_path: str
) -> int:
    timestamp = now_str()
    with db_connection() as conn:
        cursor = conn.cursor()
...
def upsert_walnut_image_feature(
    ...
    with db_connection() as conn:
        ...
        row = cursor.execute(
            "SELECT id FROM walnut_image_features WHERE image_id = ?", (image_id,)
        ).fetchone()
        return int(row["id"])
...
def list_walnut_image_features(
    walnut_id: int, feature_version: str | None = None
) -> list[dict[str, Any]]:
...
def upsert_walnut_mesh(walnut_id: int, original_filename: str, stored_path: str) -> int:
    ...
    with db_connection() as conn:
        ...
        row = cursor.execute(
            "SELECT id FROM walnut_meshes WHERE walnut_id = ?", (walnut_id,)
        ).fetchone()
        return int(row["id"])
...
def upsert_walnut_mesh_feature(
    ...
    with db_connection() as conn:
        ...
        row = cursor.execute(
            "SELECT id FROM walnut_mesh_features WHERE mesh_id = ?", (mesh_id,)
        ).fetchone()
        return int(row["id"])
...
def list_locked_pairs(
    variety_id: int | None = None, active_only: bool = True
) -> list[dict[str, Any]]:
    query = """
        SELECT lp.*,
...
def lock_pair(variety_id: int, walnut_id_1: int, walnut_id_2: int) -> int:
    ...
    with db_connection() as conn:
        ...
        cursor.execute(
            "UPDATE walnuts SET is_locked = 1, updated_at = ? WHERE id IN (?, ?)",
...
def _validate_pair_variety(
    cursor, variety_id: int, walnut_id_1: int, walnut_id_2: int
) -> None:
    rows = cursor.execute(
