...
其中 `pairnut.db` 保存核桃、品种、配对记录和图片特征值；导入图片会放在该目录下的 `images/`。后续如加入可选 AI 模型，模型文件会放在同一数据目录下的 `models/`，由需要该功能的用户自行下载。升级或重装软件时，不要删除这个数据目录。应用菜单中的“文件 -> 打开数据目录”可以直接打开该文件夹，方便用户备份或管理图片。
