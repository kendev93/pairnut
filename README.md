...
其中 `pairnut.db` 保存核桃、品种、配对记录、图片特征值和 3D 模型特征值；导入图片会放在该目录下的 `images/`，导入 3D 模型会放在 `meshes/`。当前图片匹配使用 OpenCV 特征，不需要下载模型。后续如加入可选 AI 模型，模型文件会放在同一数据目录下的 `models/`，由需要该功能的用户自行下载或在模型管理中下载。升级或重装软件时，不要删除这个数据目录。应用菜单中的“文件 -> 打开数据目录”可以直接打开该文件夹，方便用户备份或管理图片和模型文件；“文件 -> 模型管理”用于查看、下载、删除和切换特征模型，“文件 -> 打开模型目录”用于管理模型文件。
...
在“核桃管理”里选中一颗核桃后，可以点击“导入3D模型”导入 `STL`、`OBJ` 或 ASCII `PLY` 文件。每颗核桃当前保留一个 3D 模型，再次导入会覆盖旧模型。

导入成功后，应用会提取长宽高比例、体积、表面积、紧致度、球形度和表面半径波动等基础形状特征。候选配对会在双方都有 3D 特征时显示 `3D 相似度`，并把图片和 3D 这些可选证据以较小权重加入总分；没有图片或 3D 模型时，仍按基础尺寸、重量和瑕疵进行配对。

## 模型管理

“文件 -> 模型管理”会显示当前可用的特征模型。当前内置的是 `OpenCV 基础特征`，无需下载模型文件；可选 AI 模型会以模型清单形式出现，并展示预估大小、资源消耗和预期效果，方便用户判断下载代价。发布下载地址后用户可直接下载到数据目录下的 `models/`，也可以手动把模型文件放入该目录再启用。删除可选模型只会删除本地模型文件，不会删除核桃图片或数据库。切换模型后，需要重新提取已有图片特征，才能让候选配对使用新模型的特征值。
